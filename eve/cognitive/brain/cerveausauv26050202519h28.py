...
import json
import hashlib
import re # <--- ASSUREZ-VOUS QUE CET IMPORT EST PRÉSENT
import tempfile
import sqlite3
...
alma_core = None
try:
    from .Core import core as alma_core_imported_module
    alma_core = alma_core_imported_module
    print("DEBUG SCRIPT: Import 'from Core import core as alma_core' RÉUSSI.")
...
class KnowledgeBase:
    ...
    def _initialize_nlp_resources(self) -> None:
        ...
        # Ces variables globales ont été initialisées à None au début de cerveau.py.
        # Si leur import initial a réussi, elles ont déjà la bonne valeur.
        # Si leur import initial a échoué, on retente ici pour les rendre disponibles si la config le demande.

        if self.nlp_config.get("use_nltk_if_available", True):
            if not nltk: # Si la globale nltk est toujours None
                try:
                    import nltk as nltk_lib
                    nltk = nltk_lib # Assignation à la globale
                    self.logger.info("NLTK ré-assigné globalement avec succès.")
                except ImportError:
                    self.logger.warning("NLTK activé dans config mais échec de l'import (même en fallback).")
            if nltk:
                 self.logger.info("NLTK est disponible pour utilisation.")
                 # Le téléchargement des ressources NLTK est géré par enquete_alma_complete.sh ou manuellement.
            else:
                 self.logger.warning("NLTK activé dans config mais non disponible.")


        if self.nlp_config.get("use_rake_if_available", True):
            if not rake_nltk:
                try:
                    from rake_nltk import Rake
                    rake_nltk = Rake # Assignation de la classe à la globale
                    self.logger.info("Rake-NLTK (classe) ré-assigné globalement avec succès.")
                except ImportError:
                    self.logger.warning("Rake-NLTK activé dans config mais échec de l'import.")
            if rake_nltk:
                self.logger.info("Rake-NLTK (classe) est disponible pour utilisation.")
            else:
                self.logger.warning("Rake-NLTK activé dans config mais non disponible.")

        if self.nlp_config.get("use_yake_if_available", True):
            if not yake:
                try:
                    import yake as yake_lib
                    yake = yake_lib # Assignation du module à la globale
                    self.logger.info("YAKE! (module) ré-assigné globalement avec succès.")
                except ImportError:
                    self.logger.warning("YAKE! activé dans config mais échec de l'import.")
            if yake:
                self.logger.info("YAKE! (module) est disponible pour utilisation.")
            else:
                self.logger.warning("YAKE! activé dans config mais non disponible.")

        if self.nlp_config.get("use_sklearn_tfidf_if_available", True):
            if not sklearn_tfidf:
                try:
                    from sklearn.feature_extraction.text import TfidfVectorizer
                    sklearn_tfidf = TfidfVectorizer # Assignation de la classe à la globale
                    self.logger.info("Scikit-learn TfidfVectorizer (classe) ré-assigné globalement avec succès.")
                except ImportError:
                    self.logger.warning("Scikit-learn TF-IDF activé dans config mais échec de l'import.")
            if sklearn_tfidf:
                self.logger.info("Scikit-learn TfidfVectorizer (classe) est disponible pour utilisation.")
            else:
                self.logger.warning("Scikit-learn TF-IDF activé dans config mais non disponible.")
...
class PipelineStepInterface:
    ...
    def __init__(self, step_config: Dict[str, Any], global_config: Dict[str, Any], kb_instance: KnowledgeBase):
        ...
        self.text_improver: Optional[alma_core.TextImprover] = None
        self.knowledge_linker: Optional[alma_core.KnowledgeLinker] = None
...
class ComprehensionStep(PipelineStepInterface):
    def __init__(self, step_config: Dict[str, Any], global_config: Dict[str, Any], kb_instance: KnowledgeBase):
        ...
        if rake_nltk and nlp_config.get("use_rake_if_available", False):
            try:
                self.rake_instance = rake_nltk.Rake()
                self.logger.debug("Instance Rake-NLTK créée avec succès.")
            except Exception as e_rake:
                self.logger.warning(f"Impossible d'initialiser Rake-NLTK: {e_rake}", exc_info=True)
        if yake and nlp_config.get("use_yake_if_available", False):
            try:
                lang = nlp_config.get("default_language", "fr")[:2]
                n_yake = 3; dedupLim_yake = 0.9; top_yake = 10
                self.yake_extractor = yake.KeywordExtractor(lan=lang, n=n_yake, dedupLim=dedupLim_yake, top=top_yake, features=None)
                self.logger.debug(f"Instance YAKE! créée (lang: {lang}, n: {n_yake}, top: {top_yake}).")
            except Exception as e_yake:
                self.logger.warning(f"Impossible d'initialiser YAKE!: {e_yake}", exc_info=True)
    ...
    class AlmaKnowledgeEventHandler(FileSystemEventHandler): # FileSystemEventHandler doit être importé globalement
        ...
        if watchdog_should_be_enabled_in_config and WATCHDOG_AVAILABLE: # WATCHDOG_AVAILABLE est la globale
            ...
            try:
                ...
                self.watchdog_observer = Observer() # Créer une nouvelle instance d'Observer

                # Instancier la classe interne AlmaKnowledgeEventHandler (définie au niveau de CerveauService)
                # 'self' ici est l'instance de CerveauService
    ...
    def _check_service_health(self) -> None:
        ...
        if watchdog_should_be_running:
            if self.watchdog_observer and self.watchdog_observer.is_alive():
                ...
                if not self.running.is_set():
                    ...
                    try:
                        self._init_file_monitoring() # Cette méthode (re)lance watchdog et le scanner si besoin
        ...
        if self.scanner_thread and not self.scanner_thread.is_alive():
            ...
            if not self.running.is_set():
             ...
             all_ok = False
             try:
                 self._init_file_monitoring() # Cela devrait le lancer
