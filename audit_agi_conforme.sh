...
CONFORMITÉ:
  ...
  ✅ Respect total des directives AGI.md
    ...
    parser.add_argument("--target", type=str, required=True,
                       help="Répertoire à auditer (requis)")
    ...
    audit_group.add_argument("--quick", action="store_true",
                           help="Audit rapide (lignes + syntaxe uniquement)")
    audit_group.add_argument("--full-audit", action="store_true",
                           help="Audit constitutionnel complet (474 directives)")

    # Options de sortie
    parser.add_argument("--format", choices=["console", "json", "all"],
                       default="console", help="Format de rapport")

    parser.add_argument("--verbose", "-v", action="store_true",
                       help="Mode verbeux avec détails d'exécution")

    parser.add_argument("--no-color", action="store_true",
                       help="Désactive les couleurs dans la sortie")
...
def handle_environment_validation() -> int:
    ...
    try:
        ...
        if detector.validate_python_environment():
            ...
            print(f"⚙️ Capacités: {sum(capabilities.values())}/{len(capabilities)} disponibles")
